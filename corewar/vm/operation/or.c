...
/*   Updated: 2018/03/31 03:25:56 by pgerbaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../vm.h"

void	op_or_exec(t_vm *vm, t_process *process, unsigned char bin_code,
t_pc start)
{
	int				value1;
	int				value2;
	...
	if (param1.type && param2.type && param3.type)
	{
		...
		if (vm->param_v)
			verbose_op_or(process, value1, value2, param3.value);
	}
}
...
void	op_or(t_vm *vm, t_process *process)
{
	...
	increment_pc(vm, &(process->pc), 1);
	if (check_bin_code(bin_code, 7))
		op_or_exec(vm, process, bin_code, start);
	else
		increment_pc_bin_code(vm, &(process->pc), bin_code, 3 + 40);
}

void	verbose_op_or(t_process *process, int param1, int param2, int reg)
{
